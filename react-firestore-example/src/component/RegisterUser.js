...
import RegisterGame from "./RegisterTable"
  ...
  addUser = () => {
    ...
    db.collection("users")
      ...
      .then(() => {
        ...
        this.setState({ registerSuccess: true, pid: pid, username: this.state.formValues.name });
      })
      .catch(error => {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div hidden={!registerSuccess}>
           <TableMenu pid={`${this.state.pid}`} username={`${this.state.username}`}/>
        </div>
      </div>
    );
  }
